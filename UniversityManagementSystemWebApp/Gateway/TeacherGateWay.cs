...
namespace UniversityManagementSystemWebApp.Gateway
{
    public class TeacherGateway : BaseGateway
    {
        ...
        public double GetCreditTakenSumOfTeacherByTeacherId(int teacherId)
        {
            ...
            string query = "SELECT CourseId FROM CourseAssignToTeacher WHERE TeacherId = @teacherId AND Action = @insert";
            Command = new SqlCommand(query, Connection);
            Command.Parameters.AddWithValue("@teacherId", teacherId);
            ...
            if (Reader.HasRows)
            {
                Reader.Close();
                string query2 =
                    "SELECT SUM(Credit) FROM Course INNER JOIN CourseAssignToTeacher ON Course.CourseId = CourseAssignToTeacher.CourseId WHERE CourseAssignToTeacher.TeacherId=@teacherId AND Action = @insert";
        ...
        {
            string query = "SELECT * FROM CourseAssignToTeacher WHERE CourseId = @courseId AND Action = @insert";
            Command = new SqlCommand(query, Connection);
            Command.Parameters.AddWithValue("@courseId", assignCourse.CourseId);
            Command.Parameters.AddWithValue("@insert", "insert");
            Connection.Open();
